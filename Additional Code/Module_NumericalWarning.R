...
numberWarning <- function(input, output, session, Min = 0, Max = NULL) {
  ...
  observe({
    ...
    if (input$warningInput < Min) {
      ...
      id <<- showNotification(HTML("<strong>Warning</strong><br>An invalid value has been entered into the model. The model results have not been updated in order to prevent errors. Model results should be interpreted with caution while this message is displayed."), duration = 0, type = "error", closeButton = FALSE)
    }
    else if(!is.null(Max)){
